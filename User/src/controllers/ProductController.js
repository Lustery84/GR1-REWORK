app.controller("ProductController", function ($scope, $window, $rootScope, DataServices, $routeParams) {
    $rootScope.title = 'HUST Shop | Sản phẩm';
    $window.scrollTo(0, 0);
    ...
    $scope.applyFiltersAndSort = function () {
        var filteredProducts = $scope.allProducts;
        ...
        if ($scope.minPrice) {
            filteredProducts = filteredProducts.filter(p => {
                const finalPrice = p.sale > 0 ? p.price * (1 - p.sale / 100) : p.price;
                return finalPrice >= $scope.minPrice;
            });
        }
        if ($scope.maxPrice) {
            filteredProducts = filteredProducts.filter(p => {
                const finalPrice = p.sale > 0 ? p.price * (1 - p.sale / 100) : p.price;
                return finalPrice <= $scope.maxPrice;
            });
        }
