    ...
    $scope.updateStatus = function (order) {
        if (order.status === 'Đã Hủy') {
            swal({
                ...
                content: "input",
                icon: "warning",
                buttons: ["Hủy Bỏ", "Xác Nhận Hủy"],
                dangerMode: true,
            }).then((val) => {
                if (val === null) {
                    // Nếu cancel, reload lại trang để reset trạng thái
                    $window.location.reload();
                    return;
                }
                executeStatusUpdate(order, val || "Quản trị viên từ chối");
            });
        } else {
            executeStatusUpdate(order, "");
        }
    }

    function executeStatusUpdate(order, cancelReason) {
        var status = {
            status: order.status,
